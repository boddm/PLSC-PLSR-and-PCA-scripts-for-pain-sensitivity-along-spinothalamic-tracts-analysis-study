function [min_subj_per_group, is_boot_samples, boot_samples, new_num_boot] = rri_boot_check(num_subj_lst, num_cond, num_boot, incl_seq)
% Purpose: Check if bootstrap samples are needed
% Inputs:
%   num_subj_lst: vector of number of subjects per group
%   num_cond: number of conditions
%   num_boot: number of bootstrap samples
%   incl_seq: include sequential order (default=0)
% Output:
%   min_subj_per_group: minimum number of subjects per group
%   is_boot_samples: vector of 1s and 0s, 1 if bootstrap samples are needed for that group
%   boot_samples: cell array of bootstrap samples
%   new_num_boot: new number of bootstrap samples

%% check if bootstrap samples are needed
